    ...
    return async function scenario(initialContext, cb) {
      ...
      try {
        ...
        page.on("domcontentloaded", async (page) => {
          ...
          if (
            uniquePageLoadToTiming[page.url() + performanceTiming.connectStart]
          ) {
            return;
          } else {
            uniquePageLoadToTiming[
              page.url() + performanceTiming.connectStart
            ] = performanceTiming;
          }
          ...
          events.emit(
            "counter",
            `engine.browser.page.domcontentloaded.${page.url()}`,
            1
          );
          events.emit(
            "histogram",
          ...
          events.emit(
            "histogram",
            `engine.browser.page.dominteractive.${page.url()}`,
            startToInteractive
          );
        });
